...
class PipelineDefinitionValidatorTest {
    ...
    void requiresAtLeastOneRoute() {
        var pipeline = new PipelineDefinition("orders", 1, "order.created",
    ...
    void rejectsInsecureExternalDestination() {
        var pipeline = new PipelineDefinition("orders", 1, "order.created",
                List.of(new PipelineStepDefinition("route", null, null, TargetType.EXTERNAL_HTTP.name(),
                        "http://internal.example/events", Map.of())));
