...
public class HttpEnrichmentGateway implements EnrichmentGateway {
    ...
    public HttpEnrichmentGateway(RestClient.Builder builder,
    ...
    public JsonNode enrich(String source, JsonNode payload) {
        ...
        if (key.isMissingNode() || key.asText().isBlank()) {
            throw new PipelineException("ENRICHMENT_KEY_MISSING", "customerId is required for enrichment", false);
        }
        try {
            ...
            throw new PipelineException("ENRICHMENT_UNAVAILABLE", "Enrichment source is unavailable: " + source,
                    true, exception);
        }
    }
}
