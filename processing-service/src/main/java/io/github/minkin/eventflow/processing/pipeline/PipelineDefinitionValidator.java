...
public class PipelineDefinitionValidator {
    ...
    public void validate(PipelineDefinition pipeline) {
        ...
        if (pipeline.steps().isEmpty() || pipeline.steps().size() > MAX_STEPS) {
            throw new IllegalArgumentException("Pipeline must contain between 1 and " + MAX_STEPS + " steps");
        }
        ...
        for (int index = 0; index < pipeline.steps().size(); index++) {
            ...
            switch (type) {
                ...
                case "route" -> {
                    ...
                    TargetType target;
                    try {
                        target = TargetType.valueOf(required(step.target(), "steps[" + index + "].target")
                    ...
                    if (target == TargetType.EXTERNAL_HTTP) {
                        ...
                        if (!"https".equalsIgnoreCase(uri.getScheme()) || uri.getHost() == null) {
                            throw new IllegalArgumentException("External HTTP destination must be an absolute HTTPS URL");
                        }
                    } else {
