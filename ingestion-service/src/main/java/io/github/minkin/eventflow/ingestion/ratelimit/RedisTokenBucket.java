...
public class RedisTokenBucket {
    ...
    public Decision consume(String clientKey) {
        try {
            ...
            List<?> result = redis.execute(script, List.of("eventflow:rate:" + clientKey),
                    Integer.toString(capacity), Integer.toString(refillPerSecond),
                    Long.toString(System.currentTimeMillis()), Long.toString(ttlMillis));
            ...
            return new Decision(((Number) result.get(0)).longValue() == 1,
                    ((Number) result.get(1)).longValue(), false);
