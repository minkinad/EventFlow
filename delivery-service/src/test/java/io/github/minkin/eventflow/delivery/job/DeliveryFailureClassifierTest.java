...
class DeliveryFailureClassifierTest {
  ...
  void classifiesTerminalAndTransientHttpFailures() {
    assertThat(classifier.isRetryable(new HttpClientErrorException(HttpStatus.BAD_REQUEST)))
        .isFalse();
    assertThat(classifier.isRetryable(new HttpClientErrorException(HttpStatus.TOO_MANY_REQUESTS)))
        .isTrue();
    assertThat(classifier.isRetryable(new HttpServerErrorException(HttpStatus.SERVICE_UNAVAILABLE)))
        .isTrue();
