...
public class ClickHouseDeliveryAdapter implements DeliveryAdapter {
  ...
  public ClickHouseDeliveryAdapter(
      ...
      @Value("${eventflow.clickhouse.password}") String password) {
    this.client =
        builder
            .baseUrl(url)
    ...
    try {
      ...
      client
          ...
          .uri(
              uri ->
                  uri.path("/")
                      .queryParam("query", "INSERT INTO eventflow.events FORMAT JSONEachRow")
                      .queryParam("date_time_input_format", "best_effort")
                      .build())
