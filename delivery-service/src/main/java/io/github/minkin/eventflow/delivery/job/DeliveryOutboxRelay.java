  ...
  public void publish() {
    ...
    for (Message message : claimInTransaction(owner)) {
      int renewed =
          jdbc.sql(
                  """
                    UPDATE delivery_outbox SET claimed_until=now() + interval '30 seconds'
      ...
      try {
        ...
        String error = "KAFKA_PUBLICATION_FAILED";
        jdbc.sql(
                """
            ...
            .param("id", message.id())
            .param("owner", owner)
            .param("error", error.substring(0, Math.min(error.length(), 2000)))
            .update();
      }
    }
  }
